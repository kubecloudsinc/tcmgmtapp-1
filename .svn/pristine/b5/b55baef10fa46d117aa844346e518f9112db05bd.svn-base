 ...
 *
 */
public class PSSPageLoadTimeTest extends BaseTest {
	
	private final Logger logger = LoggerFactory.getLogger(PSSPageLoadTimeTest.class);
	...
	public static Collection<String> partnerNames(){
		...
		TestConstants.logger.debug("INITIALIZING THE PARTNER LIST");
	 ...
	 * @param partnerName
	 */
	public PSSPageLoadTimeTest(String partnerName) throws Exception{

		logger.debug("INSIDE THE CONSTRUCTOR");
		logger.debug("partner name is: {}",partnerName);
	...
	public void testPageLoadTime() throws Exception {
		...
		try{
			...
			logger.debug("INITIAL USERNAME: {}",pssUserName, pssPwd);
			...
			logger.debug("AFTER LOADING CONFIG USERNAME: {}",pssUserName);
			...
			// get the time taken for LNP
			logger.debug("SWITCHING TO MAIN IFRAME workspaceViewFrame");		
			start = System.nanoTime();
		
			try{
				driver = pageLoaded.until(ExpectedConditions.frameToBeAvailableAndSwitchToIt(By.className("workspaceViewFrame")));			
				logger.debug("FOUND MAIN IFRAME workSpaceViewFrame");
				...
				logger.debug("CLICKED UseFul Links ");
			}catch(NoSuchElementException exp){
				logger.info("NO SUCH ELEMENT EXCEPTION OCCURED");
				insertions.put("time_taken_to_load_lnp", "LNP NOT LOADING");
			}catch(TimeoutException exp){
				logger.info("TIME OUT ON FINDING THE ELEMENT OCCURED");
...
// after few times of testing remove the below as it is doing double work/redundant.
//			try{
...
//				logger.info("NO SUCH ELEMENT EXCEPTION OCCURED");
//			}catch(TimeoutException exp){
//				logger.info("TIME OUT ON FINDING ELEMENT OCCURED ");
//			}		

			// choose the partner, customer as all, inventory as all.
			logger.debug("TRYING TO EXTRACT INFO FROM GLOBAL FILTERS");
			selectPartner(pageLoaded, insertions, partnerName);
			logger.debug("PARTNER SELECTION COMPLETED");
			selectCustomer(pageLoaded, insertions);
			logger.debug("CUSTOMER SELECTION COMPLETED");
			selectInventory(pageLoaded, insertions);
			logger.debug("INVENTORY SELECTION COMPLETED");
			...
			try{
				...
				logger.debug("FOUND THE CUSTOMER SUMMARY LINK");
				
				BasePage.moveOverAndClick(customerSummary);
				logger.debug("CLICKED THE CUSTOMER SUMMARY LINK");
				start = System.nanoTime();
						...
						for (WebElement iframeInstance : iframes){
							if(isCustomerSummaryFound==false){
								...
								driver.switchTo().frame(iframeInstance);	
									if(!src.isEmpty() && src.contains("CUSTOMER_SUMMARY")){
										logger.debug("FOUND THE CUSTOMER SUMMARY");
											pageLoaded.until(ExpectedConditions.presenceOfAllElementsLocatedBy(By.className("dojoxGridRowTable")));
											logger.debug("DATA GRID TABLE LOADED");
				...
				logger.info("TIME OUT FOR CUSTOMER SUMMARY WIDGET OCCURED: TimeOutTime: "+timeTakenToLoadCustomerSummary);
			}
			...
			try{
				...
				logger.debug("FOUND THE ENGINEER LINK");
				
				BasePage.moveOverAndClick(engineer);
				logger.debug("CLICKED THE ENGINEER LINK");
				start = System.nanoTime();
		...
		//   FAILED_INVENTORY_PROCESSING
									...
									logger.debug("FOUND THE FAILED INVENTORY IFRAME");
									try{
										pageLoaded.until(ExpectedConditions.presenceOfElementLocated(By.className("ssueBillboardLabel")));
										logger.debug("FAILED INVENTORY IFRAME LOADING COMPLETE");
		...
		//   CUSTOMERS_FIELD_LAST_30						
									logger.debug("FOUND FEILD NOTICE IFRAME");
									try{
										pageLoaded.until(ExpectedConditions.presenceOfElementLocated(By.className("ssueBillboardLabel")));
										logger.debug("FEILD NOTICE IFRAME LOADING COMPLETE");
										notices_loaded = true;
										stopOfWidget = System.nanoTime();
		...
		//   NEW_COLLECTIONS_AVAIL																			
									logger.debug("FOUND COLLECTIONS IFRAME");
									try{
										pageLoaded.until(ExpectedConditions.presenceOfElementLocated(By.className("ssueBillboardLabel")));
										logger.debug("COLLECTIONS IFRAME LOADING COMPLETE");
								...
								driver.switchTo().parentFrame();
								logger.debug("SWITCHED TO PARENT IFRAME");
							 }
							}else{
								break;
							}
						...
						if(widgetNotLoaded){ 
							...
							logger.debug("TIME OUT FOR ENGINEER DASHBOARD LOAD FAILURE: TimeOutTime: "+ timeTakenToLoadEngineer);
							break;
						}
					}
				}
			...
			try{
				...
				logger.debug("FOUND THE SALES MANAGER LINK");
				
				BasePage.moveOverAndClick(salesManager);
				logger.debug("CLICKED THE SALES MANAGER LINK");
		            ...
		            if(iframes.size()>=3){
						for (WebElement iframeInstance : iframes){
						 if(!(coverageLast30_loaded && coverageNext90_loaded && customersNewCollection_loaded)){
							...
							if(!src.isEmpty()){
							driver.switchTo().frame(iframeInstance);				
							if(!src.isEmpty() && src.contains("CUSTOMERS_EXPIRED_COVERAGE_90")){
								logger.debug("FOUND COVERAGE NEXT 90 IFRAME");
								try{
									pageLoaded.until(ExpectedConditions.presenceOfElementLocated(By.className("ssueBillboardLabel")));
									logger.debug("PSIRT IFRAME LOADING COMPLETE");
							...
							else if(!src.isEmpty() && src.contains("CUSTOMERS_NEW_DEVICES_FOUND")){
								logger.debug("FOUND THE NEW COLLECTION IFRAME");
								try{
									pageLoaded.until(ExpectedConditions.presenceOfElementLocated(By.className("ssueBillboardLabel")));
									logger.debug("FAILED INVENTORY IFRAME LOADING COMPLETE");
							...
							else if(!src.isEmpty() && src.contains("CUSTOMERS_EXPIRED_COVERAGE_30")){
								logger.debug("FOUND EXPIRED COVERAGE 30 IFRAME");
								try{
									pageLoaded.until(ExpectedConditions.presenceOfElementLocated(By.className("ssueBillboardLabel")));
									logger.debug("FEILD NOTICE IFRAME LOADING COMPLETE");
						...
						if(widgetNotLoaded){ 
							...
							logger.debug("TIME OUT FOR SALES MANAGER DASHBOARD LOAD FAILURE: TimeOutTime: "+ timeTakenToLoadSalesManager);
							break;
						}
					}
				}
			...
			try{
				...
				logger.debug("FOUND THE SERVICE EXECUTIVE LINK");
				
				BasePage.moveOverAndClick(serviceExecutive);
				logger.debug("CLICKED THE SERVICE EXECUTIVE LINK");
		            ...
		            if(iframes.size()>=3){
						for (WebElement iframeInstance : iframes){
							if(!(productAlerts30_loaded && devicesLDOS_loaded && coveredNotCovered_loaded)){
								...
								if(!src.isEmpty()){
								driver.switchTo().frame(iframeInstance);
								if(!src.isEmpty() && src.contains("DEVICES_PAST_LDOS")){
									logger.debug("FOUND DEVICES LDOS IFRAME");
									try{
										pageLoaded.until(ExpectedConditions.presenceOfElementLocated(By.className("ssueBillboardLabel")));
										logger.debug("DEVICES LDOS IFRAME LOADING COMPLETE");
								...
								else if(!src.isEmpty() && src.contains("COVERED_NOT_COVERED")){
									logger.debug("FOUND THE COVERED NOT COVERED IFRAME");
									try{
										pageLoaded.until(ExpectedConditions.presenceOfAllElementsLocatedBy(By.className("dojoxGridRowTable")));
										logger.debug("FAILED INVENTORY IFRAME LOADING COMPLETE");
								...
								else if(!src.isEmpty() && src.contains("PRODUCT_ALERTS_LAST_30")){
									logger.debug("FOUND PRODUCT ALERTS 30 IFRAME");
									try{
										pageLoaded.until(ExpectedConditions.presenceOfAllElementsLocatedBy(By.className("dojoxGridRowTable")));
										logger.debug("PRODUCT ALERTS 30 IFRAME LOADING COMPLETE");
						...
						if(widgetNotLoaded){ 
							...
							logger.debug("TIME OUT FOR SERVICE EXECUTIVE DASHBOARD LOAD FAILURE: TimeOutTime: "+ timeTakenToLoadServiceExecutive);
							break;
						}
					}
				}
...
//      All Alerts           AllAlerts
		...
		logger.debug("TOTAL TIME TAKEN FOR ALL STEPS SO FAR: "+TimeUnit.SECONDS.convert((stopOfWholeTest - startOfWholeTest), TimeUnit.NANOSECONDS)+ " sec");
...
//		Number total=null;
		...
		try{
			...
			start = System.nanoTime();
			logger.debug("CLICKED THE {} LINK", linkName);
//			pageLoaded.until(ExpectedConditions.presenceOfElementLocated(By.id("main.container")));
				...
				driver.switchTo().frame(iframeInstance);				
					if(src.contains(sourceString)){
						...
						if(linkName.equals("Inventory Collection")){
							...
							total = rowDTO.getTotalRows() + " rows";
							logger.debug("TOTAL ROWS IN {} DATA GRID ARE: {}",linkName, total);
						}										
			...
			if(rowDTO.isStaleElementExceptionOccured()){
			 ...
			 timeTakenToLoad = 
					new Long((TimeUnit.SECONDS.convert((stop - start), TimeUnit.NANOSECONDS)) -TestConstants.WEBDRIVER_IMPLICIT_TIMEOUT_VALUE).toString() + " sec";
			}else{
				timeTakenToLoad = new Long(TimeUnit.SECONDS.convert((stop - start), TimeUnit.NANOSECONDS)).toString() + " sec";
			}
	...
	private TotalRowsDTO getTotalFromLink(WebDriverWait pageLoaded, boolean isNoRecordsDisplayed){
//		Number total=0;
		TotalRowsDTO rowDTO = new TotalRowsDTO("No Rows", false);
		long startStaleElementTime=0;
		if(isNoRecordsDisplayed==false){
			...
			while(isCountObtained==false){
				...
				WebElement spanElement =  pageLoaded.ignoring(StaleElementReferenceException.class)
						.until(ExpectedConditions.presenceOfElementLocated(By.className("toolbarItemCountSubtitle")));
//				logger.debug("The total count is: "+spanElement.getText());
				startStaleElementTime = System.nanoTime();
				
				if (spanElement!=null){
//					BasePage.moveOverAndClick(spanElement);
