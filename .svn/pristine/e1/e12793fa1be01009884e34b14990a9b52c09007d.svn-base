 ...
 *
 */
public class CopyOfPSSPageLoadTimeTest extends BaseTest {
	
	protected static final Logger logger = LoggerFactory.getLogger(CopyOfPSSPageLoadTimeTest.class);
	...
	public void testPageLoadTime() throws Exception {
		try{
			...
			//TODO remove hardcoding of the partners list and try to get from config file
			// get the customre and inventory filter info 
			logger.info("INITIALIZING THE PARTNER LIST");
			String [] partnerNames = new String [] {
					"ATEA AS",
					"CONSCIA AS",
	...
	for (int i=0;i<partnerNames.length;i++){
			...
			logger.info("STARTING THE LOAD TIME CAPTURING FOR PARTNER: {}", partnerNames[i]);
			try{
			...
			selectPartner(pageLoaded, insertions, partnerNames[i]);
...
//      All Alerts           AllAlerts
		...
		String [] toAddresses = TestConstants.PSS_TO_EMAIL_ADDRESSES.split(",");
		String [] ccAddresses = null;
		try {
			po.sendEmail(TestConstants.PSS_PAGE_LOAD_REPORT_TEMPLATE_NAME, insertions, toAddresses, ccAddresses);
